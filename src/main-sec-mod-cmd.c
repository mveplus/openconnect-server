...
int session_open(main_server_st * s, struct proc_st *proc, const uint8_t *cookie, unsigned cookie_size)
{
	...
	if (msg->explicit_local_ipv4) {
		proc->config.explicit_local_ipv4 = talloc_strdup(proc, msg->explicit_local_ipv4);
	}

	if (msg->explicit_local_ipv6) {
		proc->config.explicit_local_ipv6 = talloc_strdup(proc, msg->explicit_local_ipv6);
	}
